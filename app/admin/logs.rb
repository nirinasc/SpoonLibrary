ActiveAdmin.register Log do
  ...
  form do |f|
    inputs do
      ...
      if f.object.new_record?
        f.input :loan, as: 'select', collection: Log.book_loan.unreturned.collect { |book_loan| [book_loan, book_loan.id] },
                       allow_blank: true
      else
  ...
  # custom book return create page
  member_action :book_return, method: %i[get post] do
    @page_title = 'Add Book Return'

    classification = Log.classifications[:book_return]

    if request.post?
      data =  params.require(:log)
                    .permit(:date)
                    .merge('classification': classification, 'user': resource.user, 'book': resource.book, 'loan': resource)
